...
const saveExpenses = async (email: string, rowData: any[]) => {
  ...
  try {
    await conn.execute("DELETE FROM user_resume_cards WHERE email = ?", [
      email,
    ]);

    for (const data of rowData) {
      await conn.execute(
        "INSERT INTO user_resume_cards (email, credit_cards, value) VALUES (?, ?, ?)",
        [email, data.credit_cards, data.value]
      );
    }
